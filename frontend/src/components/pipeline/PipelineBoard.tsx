...
export function PipelineBoard({ query, stageScope, onEdit, onRequestDelete }: Props) {
  ...
  const byStage = (stage: PipelineStageId) =>
    filtered.filter((l) => String(l?.pipelineStage || 'Lead') === stage);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      console.error('[PipelineBoard] move failed', err);
    }
  };
          ...
          return (
            <div key={stage} className="snap-start shrink-0 h-full flex">
              <PipelineColumn stage={stage} totalValue={sum} count={rows.length}>
                {rows.map((lead) => (
                  <PipelineCard key={lead?._id || lead?.id} lead={lead} onEdit={onEdit} onRequestDelete={onRequestDelete} />
                ))}
              </PipelineColumn>
            </div>
          );
